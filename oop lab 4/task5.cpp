...
public:
    ...
    car(string b, string m, float r, bool a, string id):brand(b), model(m), rentalprice(r), available(a), totalrevenue(0.0), carID(id){}
    car(const car &c) : brand(c.brand), model(c.model), rentalprice(c.rentalprice), available(c.available), totalrevenue(c.totalrevenue), carID(c.carID + "_COPY") {}
    ~car()
    {
...
int main()
{
    ...
    car c1("toyota", "corolla", 100, true, "TabDG");
    car c2 = c1;
